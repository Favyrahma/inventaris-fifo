  <?php
    ...
    <div class="container-fluid py-4 px-4">
      ...
      <div class="alert alert-success alert-dismissible d-flex align-items-center justify-content-between gap-2 flex-wrap">
        ...
        <div class="d-flex gap-2 flex-shrink-0">
          ...
          <?php
            $uang_get = intval($_POST['uang_diterima'] ?? 0);
            $kem_get  = $uang_get > 0 ? max(0, $uang_get - intval($_POST['total_nilai'] ?? 0)) : 0;
            // hitung total_nilai dari items_json karena $total_nilai sudah di-scope berbeda
            $items_posted = json_decode($_POST['items_json'] ?? '[]', true);
            $tnilai_post  = 0;
            foreach ((array)$items_posted as $ip) $tnilai_post += floatval($ip['subtotal'] ?? 0);
            $kem_get = $uang_get > 0 ? max(0, $uang_get - $tnilai_post) : 0;
            $struk_extra = ($uang_get > 0)
                ? '&uang=' . $uang_get . '&kembalian=' . intval($kem_get)
                : '';
          ?>
          <a href="struk.php?id=<?= $last_trx_id ?>&auto=1<?= $struk_extra ?>" target="_blank"
            class="btn btn-sm btn-warning text-dark fw-semibold">
            <i class="bi bi-printer-fill me-1"></i>Cetak Struk
          </a>
          <a href="struk.php?id=<?= $last_trx_id ?><?= $struk_extra ?>" target="_blank"
          ...
          <div class="card">
            ...
            <div class="card-body">
              ...
              <form method="POST" id="formTrx">
                ...
                <input type="hidden" name="uang_diterima" id="uangDiterimaInput">
                <div class="mb-3">
                ...
                <!-- Bayar & Kembalian -->
                <div class="card border-0 bg-light rounded-3 p-3 mb-3" id="bayarBox" style="display:none!important">
                  <div class="row g-2 align-items-center">
                    <div class="col-12">
                      <div class="d-flex justify-content-between align-items-center mb-2">
                        <span class="fw-semibold small text-muted text-uppercase">Total Tagihan</span>
                        <span class="fw-bold fs-5 text-primary" id="totalTagihan">Rp 0</span>
                      </div>
                      <label class="form-label fw-semibold small mb-1">
                        <i class="bi bi-cash me-1"></i>Uang Diterima
                      </label>
                      <div class="input-group input-group-sm mb-2">
                        <span class="input-group-text">Rp</span>
                        <input type="number" id="uangDiterima" class="form-control fw-bold"
                              placeholder="0" min="0" step="1000"
                              oninput="hitungKembalian()" onchange="hitungKembalian()">
                      </div>
                      <!-- Tombol nominal cepat -->
                      <div class="d-flex flex-wrap gap-1 mb-2" id="nominalCepat"></div>
                      <!-- Kembalian -->
                      <div id="kembalianBox" class="rounded-2 px-3 py-2 text-center" style="background:#d1fae5;display:none">
                        <div class="small text-success fw-semibold">Kembalian</div>
                        <div class="fs-4 fw-bold text-success" id="nilaiKembalian">Rp 0</div>
                      </div>
                      <div id="kurangBox" class="rounded-2 px-3 py-2 text-center" style="background:#fee2e2;display:none">
                        <div class="small text-danger fw-semibold">Uang Kurang</div>
                        <div class="fs-4 fw-bold text-danger" id="nilaiKurang">Rp 0</div>
                      </div>
                    </div>
                  </div>
                </div>
    ...
    function renderKeranjang() {
        ...
        // Update total tagihan di bayar-box
        var bayarBox = document.getElementById('bayarBox');
        var totalTagEl = document.getElementById('totalTagihan');
        if (keranjang.length > 0) {
            bayarBox.style.setProperty('display','block','');
            totalTagEl.textContent = formatRp(sum);
            buatNominalCepat(sum);
        } else {
            bayarBox.style.setProperty('display','none','important');
        }
        hitungKembalian();
    }
    ...
    function prepareSubmit() {
        ...
        var total = keranjang.reduce(function(s,k){ return s + k.qty * k.harga; }, 0);
        var uang  = parseFloat(document.getElementById('uangDiterima').value) || 0;
        if (uang > 0 && uang < total) {
            alert('Uang diterima kurang! Total tagihan: ' + formatRp(total));
            return false;
        }
        ...
        document.getElementById('uangDiterimaInput').value = uang > 0 ? uang : '';
        return true;
    }
    ...
    function hitungKembalian() {
        ...
        var kBox  = document.getElementById('kembalianBox');
        var kuBox = document.getElementById('kurangBox');
        if (uang <= 0) {
            kBox.style.display  = 'none';
            kuBox.style.display = 'none';
        } else if (kembalian >= 0) {
            document.getElementById('nilaiKembalian').textContent = formatRp(kembalian);
            kBox.style.display  = 'block';
            kuBox.style.display = 'none';
        } else {
            document.getElementById('nilaiKurang').textContent = formatRp(Math.abs(kembalian));
            kBox.style.display  = 'none';
            kuBox.style.display = 'block';
        }
    }

    function buatNominalCepat(total) {
        var container = document.getElementById('nominalCepat');
        var pecahan = [1000,2000,5000,10000,20000,50000,100000];
        // Pilih 5 nominal >= total, pembulatan ke atas
        var saran = [];
        pecahan.forEach(function(p){
            var kelipatan = Math.ceil(total / p) * p;
            if (saran.indexOf(kelipatan) < 0 && saran.length < 5) saran.push(kelipatan);
        });
        // Selalu sertakan tepat total
        if (saran.indexOf(total) < 0) saran.unshift(total);
        saran.sort(function(a,b){ return a-b; });
        container.innerHTML = saran.slice(0,5).map(function(n){
            return '<button type="button" class="btn btn-outline-secondary btn-sm py-0 px-2" ' +
                  'onclick="setUang(' + n + ')">' + formatRp(n) + '</button>';
        }).join('');
    }

    function setUang(n) {
        document.getElementById('uangDiterima').value = n;
        hitungKembalian();
    }
