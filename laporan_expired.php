  <?php
    ...
    // kritis  = HANYA kritis saja: sisa 0–$THRESHOLD_MERAH hari (tidak termasuk expired)
    // warning = HANYA peringatan saja: sisa $THRESHOLD_MERAH+1 – $THRESHOLD_KUNING hari
    // risiko  = kritis + expired (butuh tindakan segera)
    // expired = sudah lewat tanggal kedaluwarsa saja
    $f_filter = $_GET['filter'] ?? 'all';

    $where_map = [
        'all'     => "DATEDIFF(p.tgl_expired,'$tgl') <= $THRESHOLD_KUNING",
        'kritis'  => "DATEDIFF(p.tgl_expired,'$tgl') BETWEEN 0 AND $THRESHOLD_MERAH",
        'warning' => "DATEDIFF(p.tgl_expired,'$tgl') BETWEEN " . ($THRESHOLD_MERAH + 1) . " AND $THRESHOLD_KUNING",
    ...
    $filter_label = [
        ...
        'kritis'  => 'Kritis Saja (Sisa ≤ ' . $THRESHOLD_MERAH . ' Hari)',
        'warning' => 'Peringatan Saja (Sisa ' . ($THRESHOLD_MERAH + 1) . '–' . $THRESHOLD_KUNING . ' Hari)',
